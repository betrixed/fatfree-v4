...
final class Base implements ArrayAccess {
    ...
    function split($s)
    {
        return Loader::split(s);
    }

    // deprecation support
    function fixslashes($s)
    {
        return Loader::fixslashes($s);
    }
    ...
    {
        return (!$evaluate) ? preg_replace_callback(
                        '/^@(\w+)((?:\..+|\[(?:(?:[^\[\]]*|(?R))*)\])*)/',
                        function ($expr) {
                            $str = '$' . $expr[1];
                            if (isset($expr[2]))
    ...
    /**
     * Prefix string in front of hive string variable
     * @param string $key
     * @param string $val
     * @return string&
     */
    ...
    function lexicon($path, $ttl = 0)
    {
        ...
        foreach ($languages as $lang)
            foreach (Loader::split($path) as $dir)
    ...
    function error($code, $text = '', array $trace = NULL, $level = 0)
    {
        ...
        if (!is_array($loggable))
            $loggable = Loader::split($loggable);
     ...
     * 	@param $kbps int
     * */
    function route(string|array $pattern, mixed $target, int $ttl = 0, int $kbps = 0)
    {
        $alias = null;
        if (is_array($pattern))
        {
        ...
        preg_match('/([\|\w]+)\h+(?:(?:@?(.+?)\h*:\h*)?(@(\w+)|[^\h]+))' .
                '(?:\h+\[(' . self::REQ_SPEC . ')\])?/u', $pattern, $parts);
        $alias = $parts[2] ?? null;

        if (!empty($alias))
        {
            if (!preg_match('/^\w+$/', $alias))
                user_error(sprintf(self::E_Alias, $alias), E_USER_ERROR);
            $this->hive['ALIASES'][$alias] = $parts[3];
        } else
        {
            $alias = $parts[4] ?? null;
            if (!empty($alias))
            {
                $p4 = $this->hive['ALIASES'][$alias] ?? null;
                if (empty($p4))
                    user_error(sprintf(self::E_Named, $alias), E_USER_ERROR);
        ...
        $p5 = $parts[5] ?? null;
        $type = empty($p5) ? 0 : constant('self::REQ_' . strtoupper($p5));
        ...
        if ($this->hive['DNSBL'] &&
                !in_array($ip,
                        is_array($this->hive['EXEMPT']) ?
                                $this->hive['EXEMPT'] :
                                Loader::split($this->hive['EXEMPT'])))
        {
            // Reverse IPv4 dotted quad
            $rev = implode('.', array_reverse(explode('.', $ip)));
            foreach (is_array($this->hive['DNSBL']) ?
                    $this->hive['DNSBL'] :
                    Loader::split($this->hive['DNSBL']) as $server)
            // DNSBL lookup
    ...
    /**
     * Return array[0] with CORS['expose']|null
     * Return array[1] with bool (preflight)
     * @return array
     */
    function sendCORS(): array
    {
        $cors = $this->hive['CORS'];
        if (isset($this->hive['HEADERS']['Origin']) && $cors['origin'])
        {
    ...
    static function arrayHeader(string $hdr, mixed $data): void
    {
        $header = $hdr;
        if (is_array($data))
        {
    ...
    static function mask_url(string $pattern, string $url, string $icase): array
    {
        if (!$url)
        {
        ...
        preg_match('/^' . $inner . '\/?$/' . $icase . 'um', $url, $args);
        $myargs = array_keys($args);
        foreach ($myargs as $key)
        {
    ...
    function route_find(int $reqType, array &$allowed): array
    {
        ...
        foreach ($routes as $pattern => $rtest)
        {
            ...
            if (!$preflight)
            {
                ...
                // Save matching route
                $this->hive['ALIAS'] = $handler->alias;
                $this->hive['PATTERN'] = $pattern;
                // delayed from sendCORS() ?
                if ($cors && isset($cors['expose']))
                {
                    self::arrayHeader('Access-Control-Expose-Headers: ', $cors['expose']);
                }
                if (is_string($handler->target))
                {
                    // Replace route pattern tokens in handler if any
                    $handler->target = preg_replace_callback('/({)?@(\w+\b)(?(1)})/',
                            function ($id) use ($args) {
                                $arg1 = $id[2] ?? $id[1];
                                return $args[$arg1] ?? $id[0];
                            },
                            $handler->target);

                    if (preg_match('/(.+)\h*(?:->|::)/', $handler->target, $match))
                    {
    ...
    function run(): mixed
    {
        $this->route_time = microtime(true);
        ...
        $isAjax = $this->hive['AJAX'];

        if ($isCLI)
        {
            $reqType = self::REQ_CLI;
        } else
        {
            $reqType = $isAjax ? self::REQ_AJAX : self::REQ_SYNC;
        }

        $allowed = [];

        list($handler, $cors) = $this->route_find($reqType, $allowed);
        ...
        {
            $ttl = $handler->ttl;
            if ($ttl && ($verb === 'GET' || $verb === 'HEAD'))
            {
                $headers = $this->hive['HEADERS'];
                $cache = Cache::instance();
                $hash_id = $this->hash($verb . ' ' . $this->hive['URI']) . 'url';
                $cached = $cache->exists($hash_id, $data);

                if ($cached)
                {
                    $mod_time = $headers['If-Modified-Since'] ?? null;
                    if ($mod_time && strtotime($mod_time) + $ttl > $now)
                    {
                        $this->status(304);
                        die();
                    }
            ...
            if (!strlen($body))
            {
                ...
                ob_start();
                $args = $this->hive['PARAMS'];
                $result = $this->call($handler->target, [$this, $args, $handler->target],
        ...
        } elseif (!$isCLI)
        {
            ...
            foreach($allowed as $h) {
                if (is_string($h->verb)) {
                    ...
                    $verbs = array_merge($verbs,$h->verbs);
                }
            }
            if (!preg_grep('/Allow:/', $headers_send))
            {
                // Unhandled HTTP method
                header('Allow: ' . implode(',', array_unique($verbs)));
            }
            if ($cors)
            {
                ...
                if ($cors['headers'] && !preg_grep('/Access-Control-Allow-Headers:/'
                                , $headers_send))
                {
                    self::arrayHeader('Access-Control-Allow-Headers: ', $cors['headers']);
                }
                if ($cors['ttl'] > 0)
                {
    ...
    {
        $out = [];
        foreach (is_array($funcs) ? $funcs : Loader::split($funcs) as $func)
            $out[] = $this->call($func, $args);
        return $out;
    }
